import { v4 } from 'uuid';
import {
  Collection, Configuration, EntityManager, LockMode, MikroORM, QueryFlag, QueryOrder, Reference, Logger, ValidationError, wrap, UniqueConstraintViolationException,
...
import { initORMMsSql, wipeDatabaseMsSql } from './bootstrap';
...
describe('EntityManagerMsSql', () => {
  ...
  beforeEach(async () => wipeDatabaseMsSql(orm.em));
  ...
  test('populate OneToOne relation on inverse side', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('select top (@p0) [e0].* from [foo_baz2] as [e0] where [e0].[id] = @p1');
    expect(b0.bar).toBeUndefined();
    orm.em.clear();

    const b1 = await orm.em.findOneOrFail(FooBaz2, { id: baz.id }, ['bar']);
    expect(mock.mock.calls[1][0]).toMatch('select top (@p0) [e0].*, [e1].[id] as [bar_id] from [foo_baz2] as [e0] left join [foo_bar2] as [e1] on [e0].[id] = [e1].[baz_id] where [e0].[id] = @p1');
    expect(mock.mock.calls[2][0]).toMatch('select [e0].*, (select 123) as [random] from [foo_bar2] as [e0] where [e0].[baz_id] in (@p0) order by [e0].[baz_id] asc');
    ...
    expect(mock.mock.calls[3][0]).toMatch('select top (@p0) [e0].*, [e1].[id] as [bar_id] from [foo_baz2] as [e0] left join [foo_bar2] as [e1] on [e0].[id] = [e1].[baz_id] where [e1].[id] = @p1');
  ...
  test('populate queries respect the root condition (query condition propagation)', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('select [e0].* from [author2] as [e0] left join [book2] as [e1] on [e0].[id] = [e1].[author_id] where [e1].[title] in (@p0, @p1)');
    expect(mock.mock.calls[1][0]).toMatch('select [e0].*, [e0].price * 1.19 as [price_taxed] from [book2] as [e0] where [e0].[author_id] is not null and [e0].[author_id] in (@p0) and [e0].[title] in (@p1, @p2) order by [e0].[title] asc');
  });
  ...
  test('self referencing (1 step)', async () => {
    ...
    expect(mock.mock.calls[5][0]).toMatch('select top (@p0) [e0].*, [e1].[author_id] as [address_author_id] from [author2] as [e0] left join [address2] as [e1] on [e0].[id] = [e1].[author_id] where [e0].[id] = @p1');
  });
  ...
  test('find by joined property', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('select [e0].*, [e0].price * 1.19 as [price_taxed] ' +
      ...
      'where [e0].[author_id] is not null and [e1].[name] = @p0');
